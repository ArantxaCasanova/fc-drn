...
class ThreadedDataset(object):
    ...
    def __init__(self,
        ...
        if (not hasattr(self, 'data_shape') and batch_size > 1 and
            ...
            print (' Care, there is batch size > 1 but image sizes are different. Is multi-gpu setting enabled?')
            # raise ValueError(
        ...
        if not os.path.exists(self.path):
            ...
            if self.is_open_permissions == "True":
                ...
                os.chmod(self.datasets_local_path, 0o775)
        ...
        if self.return_01c:
            ...
            self.data_shape = [data_shape[i] for i in
                               [2] + list(range(2)) + list(range(3, len(data_shape)))]
    ...
    def _step(self):
        ...
        while not done:
            if self.use_threads:
                ...
                try:
                    ...
                    if len(data_batch) == 3:
                        ...
                        if (isinstance(data_batch[1], type(BaseException)) or
                            ...
                            raise Exception(data_batch[0], data_batch[1], data_batch[2])
                    done = True
                ...
                try:
                    name_batch = next(self.names_batches)
                    name_batch = [x for x in name_batch if x is not None]
...
def threaded_fetch(weakself):
    ...
    while True:
        ...
        try:
            ...
            # Load the data
            batch_to_load = [x for x in batch_to_load if x is not None]
