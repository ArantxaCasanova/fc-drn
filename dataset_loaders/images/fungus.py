...
class FungusDataset(ThreadedDataset):
    ...
    # optional arguments
    data_shape = (1658, 1705, 1)
    _cmap = {
        0: (0, 0, 0),  # Background
        1: (255, 255, 255)}  # Fungus
    _mask_labels = {0: 'Background', 1: 'Fungus'}
    _filenames = None
    _prefix_list = None
    def __init__(self, which_set='train', split=1, rand_perm=None, *args, **kwargs):
        ...
        if self.which_set in ["train", "val"]:
            self.image_path = os.path.join(self.path, 'train','annotated_images', 'images' )
            self.target_path =os.path.join(self.path, 'train','annotated_images', 'gt' )
        elif self.which_set == "test":
            self.image_path = os.path.join(self.path, 'test', 'images' )
            self.target_path = os.path.join(self.path, 'test', 'gt' )
            self.set_has_GT = True
        elif self.which_set == "unlabeled":
            self.image_path = os.path.join(self.path, 'train','unannotated_images' )
        ...
        super(FungusDataset, self).__init__(*args, **kwargs)
    ...
    def load_sequence(self, sequence):
        ...
        for prefix, frame in sequence:
            ...
            img =  np.array(img)[:, :, None].astype("uint8")
            X.append(img)
            F.append(frame)
